...
export const useGetMe = (opts?: { query?: { retry?: boolean } }) =>
  useQuery<User>({ queryKey: getGetMeQueryKey(), queryFn: () => apiFetch("/api/auth/me"), retry: opts?.query?.retry ?? false });

export const useLogin = () =>
  useMutation<{ user: User; token: string }, Error, { data: { email: string; password: string } }>({
    mutationFn: ({ data }) => apiFetch("/api/auth/login", { method: "POST", body: JSON.stringify(data) }),
  });
...
export const useRegister = () =>
  useMutation<{ user: User; token: string }, Error, { data: { firstName: string; lastName: string; email: string; password: string; agreedToTerms: boolean } }>({
    mutationFn: ({ data }) => apiFetch("/api/auth/register", { method: "POST", body: JSON.stringify(data) }),
  });
...
export const useListPlans = (_opts?: unknown) =>
  useQuery<Plan[]>({ queryKey: getListPlansQueryKey(), queryFn: () => apiFetch("/api/plans") });

export const getListTicketsQueryKey = () => ["listTickets"] as const;
export const useListTickets = (_opts?: unknown) =>
  useQuery<Ticket[]>({ queryKey: getListTicketsQueryKey(), queryFn: () => apiFetch("/api/tickets"), retry: false });

export const useCreateTicket = () =>
  useMutation<Ticket, Error, { data: { subject: string; message: string; priority: string } }>({
    mutationFn: ({ data }) => apiFetch("/api/tickets", { method: "POST", body: JSON.stringify(data) }),
  });

export const getGetOverviewQueryKey = () => ["getOverview"] as const;
export const useGetOverview = (_opts?: unknown) =>
  useQuery<Stats>({ queryKey: getGetOverviewQueryKey(), queryFn: () => apiFetch("/api/stats/overview") });
